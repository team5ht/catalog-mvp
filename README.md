...
- `styles/tokens.css` — базовые дизайн‑токены (цвета, шрифты, размеры, тени, отступы).
- `styles/tokens.theme-linear.css` — тема, переопределяющая токены (другая палитра/шрифт).
- `styles/base.css` — базовая типографика, контейнеры, паддинги, баннер.
- `styles/components/buttons.css` — кнопки `.btn*` и `.button*`, чипы `.chip*`.
- `styles/components/cards.css` — карточки материалов для каруселей.
- `styles/components/navigation.css` — нижняя навигация и состояние `is-loading` для профиля.
- `styles/components/scroll-containers.css` — горизонтальные карусели и лента категорий.
- `styles/pages/catalog.css` — стили каталога/поиска/карточек списка.
- `styles/pages/material.css` — стили страницы материала (обложка, кнопка назад, теги).
- `styles/pages/login.css` — стили авторизации (градиентный фон, карточка формы).
- `styles/pages/account.css` — стили личного кабинета.
- `styles/README.md` — объяснение структуры стилей и порядка подключения.
- `styles/DESIGN-CODE.md` — принципы дизайна и правила использования токенов.
...
- Навигация «Назад»:
  - Игнорирует реферер, если он ведёт на `auth-login.html`.
  - Сохраняет/читает адрес в `sessionStorage` по ключу `materialBackUrl:<id>`.
  - При отсутствии реферера использует fallback `catalog.html`.
- Загрузка данных:
...
- Кнопка «Скачать»:
  ...
  - После загрузки сессии Supabase меняет текст/стили: «Скачать» (авторизован) или «Войти и скачать» (не авторизован).
...
- `renderInlineError(containerId, message)`:
  ...
  - Класса `.load-error` в CSS **нет**, поэтому внешний вид не стилизован.
...
- Версия кэша: `catalog-mvp-v6`.
- Список `urlsToCache` включает основные страницы, скрипты, базовые стили и изображения.
  - **Не кэшируются:** `styles/tokens.theme-linear.css`, `supabase-config.js`, CDN Supabase SDK.
...
- `styles/tokens.css` — базовая система токенов (палитра, типографика, тени, отступы, токены нижней навигации).
- `styles/tokens.theme-linear.css` — активная тема проекта (переопределяет цвета, тени, радиусы и шрифт на Inter‑подобный стек).
- `styles/base.css` — базовая типографика, контейнер `.app-container` (max‑width 640px), отступы `main` с учётом нижней навигации.
- Компоненты:
  - `buttons.css`: две независимые системы кнопок (`.btn*` и `.button*`), чипы `.chip` + состояния.
  - `cards.css`: карточки материалов для каруселей, fallback для `aspect-ratio`.
  - `scroll-containers.css`: горизонтальные карусели и лента категорий без видимых скроллбаров.
  - `navigation.css`: нижняя навигация, активные/hover‑состояния, `is-loading`.
- Страницы:
  - `catalog.css`: лента каталога, карточки списка, поиск‑заглушка. Использует `box-shadow: var(--shadow-soft)` (токен **не определён** в `tokens.css`).
  - `material.css`: оформление материала и плавающей кнопки «назад». Использует `box-shadow: var(--shadow-hero)` (токен **не определён** в `tokens.css`).
  - `login.css`: градиентный фон использует `--color-hero-gradient-end` (токен **не определён** в `tokens.css`).
  - `account.css`: стили карточки аккаунта и ошибок.
- Подробные правила по токенам и дизайн‑код — в `styles/DESIGN-CODE.md`.
...
- `.load-error` не имеет CSS‑стилей.
- `styles/tokens.theme-linear.css` не входит в offline‑cache `sw.js`.
- Используются legacy‑токены `--shadow-soft`, `--shadow-hero`, `--color-hero-gradient-end`, которые не определены в `tokens.css`.
