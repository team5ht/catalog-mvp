...
- `account.html` - проверяет сессию сразу при загрузке и через `onAuthStateChange`. Без сессии уводит на `auth-login.html?redirect=<URL>`, с сессией показывает email и даёт кнопку выхода (`signOut()` + редирект на логин); выход доступен только здесь.
