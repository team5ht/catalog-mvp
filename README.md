...
- Кнопка профиля в навигации (`nav-auth.js`): проверяет сессию Supabase (`auth.getSession`/`onAuthStateChange`), подставляет иконки вход/выход и aria-label, скрыта до загрузки сессии. Нажатие незалогиненных уводит на `auth-login.html?redirect=<текущий URL>`, залогиненных — вызывает `signOut()`, обновляет иконку и показывает toast.
...
- `nav-auth.js` / `material.html` - используют Supabase: проверяют сессию, подписываются на `onAuthStateChange`, переключают UI и перенаправляют пользователей в зависимости от авторизации (вход, скачивание, signOut). Выход выполняется только через кнопку навигации.
...
Слои: `styles/tokens.css` (палитра, тени, типоразмеры, токены нижней навигации и legacy-алиасы), `styles/base.css` (базовая типографика, контейнеры, паддинги `main` с safe-area, баннер главной), компоненты (`styles/components/*`: два набора кнопок `.btn*`/`.button*` + чипы, карточки, горизонтальные скроллы/категории, нижняя навигация с `is-loading` для профиля и toast), страничные стили (`styles/pages/catalog.css`, `material.css`, `login.css`). Дизайн-принципы — в `styles/pages/DESIGN-CODE.md`, структура и советы по добавлению — в `styles/README.md`.

## PWA

- `manifest.json` с иконками (`icon-192.png`, `icon-512.png`), без скриншотов.
- `sw.js` работает под версией `catalog-mvp-v5`. `urlsToCache` включает ключевые страницы, скрипты и стили. Стратегия: same-origin GET сначала сеть с записью в кэш, при ошибке — ответ из кэша; для `navigate` в оффлайне отдаётся закешированный `index.html`. При добавлении/переименовании файлов обновляйте список и версию кэша.
