...
- Важно: `images:build` и `images:check` запускайте строго последовательно (сначала `build`, потом `check`), не параллельно.
...
Для обложек генерируются размеры: `160/240/320/480w` с ratio `3:4`
(`160x213`, `240x320`, `320x427`, `480x640`; в `.webp` и `.jpg`).
